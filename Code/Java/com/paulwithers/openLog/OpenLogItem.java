...
public class OpenLogItem implements Serializable {
	...
	public String getLogDbName() {
		if ("".equals(_logDbName)) {
			...
			_logDbName = OpenLogUtil.getXspProperty("xsp.openlog.filepath", "bookm.nsf");
	...
	/**
	 * @param ee
	 *            Throwable from which to retrieve stack trace
	 * @return the errLine
	 */
	...
	public boolean writeToLog() {
		...
		try {
			...
			logDoc.replaceItemValue("LogErrorMessage", trimString(errMsg));
			...
			Intra in = new Intra();
			String userid = in.getCookieValue("userid");
			Database cdb = getSessionAsSigner().getCurrentDatabase();
			String key = "User_USID_" + userid;
			System.out.println("Key = " + key);
			ViewEntry entry = cdb.getView("(LookupUsers)").getEntryByKey(key,true);
			if(entry!=null){
				String fn = entry.getDocument().getItemValueString("FullName");
				logDoc.replaceItemValue("LogUserName", fn + " (" + userid + ") " + getUserName());
			}else{
				logDoc.replaceItemValue("LogUserName", userid + " " + getUserName());
			}
			*/
			//System.out.println("message " + getMessage());
			//logDoc.replaceItemValue("LogUserName", getUserName());
			logDoc.replaceItemValue("LogUserName", OpenLogErrorHolder.myMsg + " " + getUserName());
			//logDoc.replaceItemValue("LogUserName", getMessage().substring(getMessage().lastIndexOf("***"))); // TA 20170829
	...
	public String trimString(String input) {
	    if (input.getBytes().length > 60000) {
	        return input.substring(0, input.indexOf(" ", 60000));
	    }
	    return input;
	}
	 ...
	 *             exception caused by lotus.domino core error
	 */
